#include <stdio.h>
 ...
 // S will have 2n possible combinations of cities, so the size of it should be
  //  2^MAXN that 1<< MAXN
 int dp[MAXN][1 << MAXN];
 ...
 int g(int i, int S)
 {
 // Base case: If S is empty, return the cost to return to the starting city
   // (A)
 if (S == 0)
return d[i][0]; //g(i,{}) = d[i][0] - Cost to return to starting city Afrom city i
...
int minCost = INF;
 ...
 // You can also start the loop from k = 1 to < n, since city 0 (A) is not
    // included in S.
 // Even if you start from k = 0, the condition S & (1 << k) will be false, so
 ...
 // Store the computed final minimum cost in dp[i][S] for current city i to
